...
for ibin = 1:nbins
	...
	% 	Append the legend
	% 
	obj.ts.BinParams.Legend_s.CLTA{ibin} = [num2str(round((binEdges(ibin)/1000),3)) 's - ' num2str(round((binEdges(ibin + 1)/1000),3)) 's | trial=' num2str(trials_in_each_bin{ibin}(n))]; %num2str(numel(trials_in_each_bin{ibin}))];
	% 
