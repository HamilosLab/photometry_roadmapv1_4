...
for ibin = 1:nbins
	...
	% ---------new: we'll try to get the trials in each bin as the next trial
	trials_in_each_bin = cellfun(@(x) x+1, trials_in_each_bin, 'uniformoutput',0);
	% need to give back the average lick time on the next trial
	mean_lick_time_next_trial(ibin) = nanmean(lick_times_next_trial{ibin});
	median_lick_time_next_trial(ibin) = nanmean(lick_times_next_trial{ibin});

	for n = 1:numel(trials_in_each_bin{ibin})
		% make sure we aren't past the end...
		if trials_in_each_bin{ibin}(n) > numel(obj.GLM.flick_s_wrtc)
			disp(['went past last trial at n=' num2str(trials_in_each_bin{ibin}(n)) '. skipping this one'])
			continue
        elseif isnan(fLick(trials_in_each_bin{ibin}(n)))
            disp('no lick on trial n+1, skipping')
            continue
		end
